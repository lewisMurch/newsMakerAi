...
command = f'cmd /c "{bat_file_path}" --/exts/omni.services.transport.server.http/port=8011'
    ...
    for domain_num in range (0, domain_amount):
        ...
        for article_num in range(0, article_amount): 
            try:
                ...
                try:
                    print(f'Error with generating article number {article_num} in right: ', e)
                except:
                    print('Error with generating article right: ', e)
...
#custom_run(' ', ' ', 'right', True, 'STIME', 30)